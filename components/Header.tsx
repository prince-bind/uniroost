          ...
          {status === 'loading' ? (
            ...
            <div className="flex items-center space-x-4">
              ...
              {session.user?.role === 'STUDENT' && (
                <Link href="/student" className="text-sm font-bold text-gray-700 hover:text-cyan-600 transition-colors">
                  Profile
                </Link>
              )}
              {session.user?.role === 'OWNER' && (
                <Link href="/owner" className="text-sm font-bold text-gray-700 hover:text-cyan-600 transition-colors">
                  Dashboard
                </Link>
              )}
              {session.user?.role === 'ADMIN' && (
                <Link href="/admin" className="text-sm font-bold text-gray-700 hover:text-cyan-600 transition-colors">
                  Admin Panel
                </Link>
              )}
              ...
              <button 
                ...
                className="text-sm font-black text-white bg-red-600 px-5 py-2 rounded-full hover:bg-red-700 hover:shadow-lg transition-all active:scale-95 cursor-pointer"
              >
                Log out
              </button>
            </div>
